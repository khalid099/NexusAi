...
Step 1: Fetch / Validate Data
   ↓
Step 2: AI Processing (Model Router → Claude / GPT / Gemini)
   ↓
...
- All AI calls go through the `AIService` model router — never call OpenAI/Claude directly from controllers
...
- `skills/` — reusable Claude Code skill prompts for common dev tasks
- `roles/` — role-based personas Claude should adopt per task context

## Active Roles
Load the appropriate role from `.claude/roles/` based on task:
...
## Active Skills
Load from `.claude/skills/` for targeted tasks:
